    ...
    public RegisterUI(UserManager userManager) {
        ...
        registerBtn.addActionListener(e -> {
            ...
            if (u.isEmpty() || p.isEmpty()) {
                showStyledDialog(this, "Error, Empty fields", "Please fill in all fields", new Color(239, 68, 68));
                return;
            }

            if (u.length() < 3) {
                showStyledDialog(this, "Username too small", "Username must be at least 3 characters", new Color(239, 68, 68));
                return;
            }

            if (p.length() < 4) {
                showStyledDialog(this, "password >= 4 char", "Password must be at least 4 characters", new Color(239, 68, 68));
                return;
            }
            boolean success = userManager.register(u, p);
        ...
        if (success) {
            ...
            Timer timer = new Timer(1000, evt -> {
                ...
                // auto-login to get User object
                User loggedInUser = userManager.login(u, p);

                if (loggedInUser != null) {
                    new DashboardUI(userManager, loggedInUser);
                } else {
                    new LoginUI(userManager); // fallback (shouldn't happen)
                }
            });
            ...
            showStyledDialog(this, "use different username", "Username already exists", new Color(239, 68, 68));
        }
